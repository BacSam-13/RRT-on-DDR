...
def look_for_padre(vertices, aristas, id_father, id_son):
    ...
    #Se asume que cada vertice tiene un único padre
...
#Recibe un conjunto de veritces para dibujarlos como poligonos, segun color del interior y frontera.
def draw_poligonos(obstaculos, color_polygon, color_border):
   global view
   for obstaculo in obstaculos:
        poligono = Polygon(obstaculo, color=color_polygon, border_color = color_border)
        view.add(poligono)
...
class RRT:
    ...
    def __init__(self, start, b, r, Dt, h, goal, obstaculos, xlim, ylim):
        ...
        self.vertices = [start]
        self.edges = []#El concepto de aristas se cambio para alamcenar indices en lugar de estados
        self.xlim = xlim
        self.ylim = ylim
        self.b = b
        self.r = r
    ...
    def visualize(self, flag):
        ...
        if flag:#Si se encontro goal
            print("\nSE ENCONTRO X_goal")
            Estados = look_for_padre(self.vertices, self.edges, 0, len(self.vertices)-1)
            print("\nSe calculo la ruta")
            return Estados
...
found_path = RRT_DDR.plan(Max_iteraciones)

print(f"\nTerminamos RRT con {len(RRT_DDR.vertices)} vertices y {len(RRT_DDR.edges)} aristas")


canvas = SceneCanvas(keys='interactive', title='RRT_on_DDR_move',
...
#Creamos x_start y x_goal
vetices_robot = create_estado(X_start, r, b)
vetices_goal = create_estado(X_goal, r, b)

START = Polygon(vetices_robot, color=cyan)
view.add(START)
START.transform = MatrixTransform()

GOAL = Polygon(vetices_goal, color=red)
view.add(GOAL)
print("\nDibujamos X_goal y X_start")

#Buscamos el vamino desde start a goal (regresa [] si no hay)
Camino = RRT_DDR.visualize(found_path)
print("\nVisualizamos RRT")

#Imprimimos el camino si hay
if found_path:
    print("\nEl camino de x_start a x_goal es:\n",Camino)
    #Ya tenemos el arreglo que guarda los estados entre x_start y x_goal
    #Lo convertiremos a un arreglo de vertices
    Vertices = []
    for x in Camino:
        Vertices.append(create_estado(x,r,b))

    #Graficamos los estados
    draw_poligonos(Vertices, None, white)

else:
    print("\nNo se encontro camino desde x_start a x_goal")
